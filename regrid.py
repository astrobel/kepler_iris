...
# def ndregrid(arr, factor, xshift, yshift):
#    new_arr = np.zeros((arr.shape[0], arr.shape[1]*factor, arr.shape[2]*factor))

#    for index in range(arr.shape[0]):
#       exes = np.arange(0, new_arr.shape[1])
#       whys = np.arange(0, new_arr.shape[2])
#       for j in range(len(xshift)):
#          xmove = np.floor(exes + xshift[j])
#          ymove = np.floor(whys + yshift[j])
#       new_arr[xmove] =  arr[index,:]

#    return new_arr

# this last function is a wip.. lol